document.addEventListener('DOMContentLoaded', () => {
    ...
    let addColl = (tr, el) => {
        ...
        if(sumOfRow){
            td.innerHTML = sumOfRow;
        }else{
            td.innerHTML = el.amount;
        }


        if (hoverValue && !deleteRowProccess) {
    ...
    let approximateValues = async(event)=>{
        ...
        if (+event.target.innerHTML !== +hoverValue) {
          //  document.documentElement.scrollTop = document.body.scrollTop = event.target.getBoundingClientRect().top;
    ...
    let addRow = async (tbl, col, step) => {
        let tr = document.createElement('tr');
        let rowSum = 0;
        for (let i = 0; i < col; i++) {
            await addColl(tr, data[step]);
        ...
        span.addEventListener('mousemove', (item)=>{
            sumOfRow = item.target.innerText;
            clearTable();
            renderTable(m, n);
        });
    ...
    let renderTable = async (row = 9, col = 9) => {
        ...
        let step = 0;
        for (let i = 0; i < row; i++) {
            step = await addRow(tbl, col, step)
        }
        await calcAvarages();
